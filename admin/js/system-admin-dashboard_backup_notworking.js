...
class SystemAdminDashboard {
    ...
    async loadClientsData() {
        try {
            ...
            if (response.ok) {
                ...
                const total = data.data?.pagination?.total || clients.length;
        ...
        return { clients: [], total: 0, activeSubscriptions: 0, adminAccounts: 0, templateAccounts: 0 };
    }
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Showing <span id="showingFrom">1</span> to <span id="showingTo">${clientsData.clients?.length || 0}</span> of <span id="totalCount">${clientsData.total || 0}</span> clients
                                </p>
                            </div>
                            <div>
                    ...
                    <div class="flex items-start space-x-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <button onclick="window.systemAdminDashboard.approveMediaWithBlur(${item.id})" 
    ...
    openBlurToolModal(mediaId, item) {
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="range" id="blurStrength" min="1" max="50" value="15" 
                                                   class="w-full" onchange="window.systemAdminDashboard.updateBlurSettings()">
                                            <span class="text-sm text-gray-500">Current: <span id="blurStrengthValue">15</span>px</span>
                                        </div>
        ...
        // Add a fallback initialization in case image onload doesn't fire
        setTimeout(() => {
            const image = document.getElementById('blurPreviewImage');
            if (image && (image.complete || image.naturalWidth > 0)) {
                console.log('Fallback: Initializing blur overlays via setTimeout');
                this.initializeBlurOverlays(mediaId);
            }
        }, 500);
    }
    ...
    initializeBlurOverlays(mediaId) {
        ...
        if (!image.complete && image.naturalWidth === 0) {
            ...
            image.addEventListener('load', () => {
                console.log('Image loaded, reinitializing overlays');
                this.initializeBlurOverlays(mediaId);
            }, { once: true });
            return;
        }
        ...
        container.style.pointerEvents = 'auto'; // Ensure container allows mouse events
    ...
    createBlurOverlay(part, location, image, container) {
        ...
        if (pos) {
            ...
            // Store original coordinates for proper scaling
            overlay.dataset.originalX = pos.x;
            overlay.dataset.originalY = pos.y;
            overlay.dataset.originalWidth = pos.width;
            overlay.dataset.originalHeight = pos.height;
            ...
            overlay.style.zIndex = '1000'; // Much higher z-index
            overlay.style.minWidth = '20px';
            overlay.style.minHeight = '20px';
            overlay.style.pointerEvents = 'auto'; // Ensure mouse events work
            ...
            // Add comprehensive test event handlers to debug mouse events
            overlay.addEventListener('click', (e) => {
                console.log(`TEST: Click detected on ${part} overlay`);
            ...
            overlay.addEventListener('mouseenter', () => {
                console.log(`TEST: Mouse entered ${part} overlay`);
            });
            
            overlay.addEventListener('mouseleave', () => {
                console.log(`TEST: Mouse left ${part} overlay`);
            });
            
            // Force cursor style
            overlay.onmouseover = () => {
                overlay.style.cursor = 'move';
                console.log(`TEST: Cursor set to move on ${part}`);
            };
    ...
    addResizeEventHandlers(handle, position, overlay) {
        ...
        const handleResize = (e) => {
            ...
            // Update dataset with new original coordinates (scaled back to image coordinates)
            const image = document.getElementById('blurPreviewImage');
            if (image) {
                const scaleX = image.naturalWidth / image.offsetWidth;
                const scaleY = image.naturalHeight / image.offsetHeight;
                
                overlay.dataset.originalX = Math.round(newLeft * scaleX);
                overlay.dataset.originalY = Math.round(newTop * scaleY);
                overlay.dataset.originalWidth = Math.round(newWidth * scaleX);
                overlay.dataset.originalHeight = Math.round(newHeight * scaleY);
                
                console.log(`Resize updated dataset: ${overlay.dataset.originalX},${overlay.dataset.originalY} ${overlay.dataset.originalWidth}x${overlay.dataset.originalHeight}`);
            }
        };
    ...
    addDragHandlers(overlay, part, image, container) {
        ...
        console.log(`Adding drag handlers to overlay for part: ${part}`);

        // Double-click to toggle blur (replacing single click)
        overlay.addEventListener('dblclick', (e) => {
            console.log(`Double-click detected on ${part} overlay`);
            e.stopPropagation();
        ...
        overlay.addEventListener('mousedown', (e) => {
            console.log(`Mouse down on ${part} overlay`);
            
            // Don't start drag if clicking on a resize handle
            if (e.target.classList.contains('resize-handle')) {
                console.log('Clicked on resize handle, not starting drag');
                return;
            }
            ...
            console.log(`Starting drag - Initial position: ${startLeft},${startTop}`);
        ...
        const handleDrag = (e) => {
            ...
            overlay.dataset.originalX = Math.round(constrainedLeft * scaleX);
            overlay.dataset.originalY = Math.round(constrainedTop * scaleY);
            
            // Debug: Log movement every 10 pixels
            if (Math.abs(deltaX) % 10 < 2 || Math.abs(deltaY) % 10 < 2) {
                console.log(`Dragging ${part}: delta(${deltaX},${deltaY}) -> position(${constrainedLeft},${constrainedTop})`);
                console.log(`Updated dataset: ${overlay.dataset.originalX},${overlay.dataset.originalY}`);
            }
        };
    ...
    async saveBlurApproval(mediaId) {
        ...
        blurredParts.forEach(part => {
            const overlay = document.querySelector(`.blur-overlay-${part}`);
            if (overlay && image) {
                console.log(`Capturing data for ${part} overlay`);
                
                // Use dataset coordinates if available (these are already in natural image coordinates)
                let naturalCoords;
                if (overlay.dataset.originalX && overlay.dataset.originalY && 
                    overlay.dataset.originalWidth && overlay.dataset.originalHeight) {
                    
                    naturalCoords = {
                        x: parseInt(overlay.dataset.originalX),
                        y: parseInt(overlay.dataset.originalY),
                        width: parseInt(overlay.dataset.originalWidth),
                        height: parseInt(overlay.dataset.originalHeight),
                    ...
                    console.log(`Using dataset coordinates for ${part}:`, naturalCoords);
                } else {
                    // Fallback to style coordinates (convert to natural image coordinates)
                    ...
                    console.log(`Using fallback style coordinates for ${part}:`, naturalCoords);
                }
