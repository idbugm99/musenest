-- Migration 009: Industry-Specific Architecture (Fixed)
...
CREATE TABLE IF NOT EXISTS business_page_sets (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
SET @sql = IF(@col_exists = 0, 
    ...
    'SELECT "page_types columns already exist"');
...
SET @sql = IF(@theme_col_exists = 0,
    ...
    'SELECT "theme_sets columns already exist"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@model_col_exists = 0,
    ...
    'SELECT "models columns already exist"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Insert initial business types
INSERT IGNORE INTO business_types (name, display_name, description, category, default_page_sets, required_features, age_verification_required, content_warnings_required) VALUES
...
INSERT IGNORE INTO business_page_sets (business_type_id, name, display_name, description, included_pages, tier, features) VALUES
...
-- Update existing page type for escort industry
UPDATE page_types SET 
    business_type_id = (SELECT id FROM business_types WHERE name = 'escort'),
    industry_specific = TRUE,
    alternative_names = JSON_OBJECT('escort', 'Rates & Services', 'general', 'Pricing')
WHERE name = 'rates';

-- Insert industry-specific page types for escort (skip rates since it exists)
INSERT IGNORE INTO page_types (name, display_name, description, category, business_type_id, industry_specific, content_structure, required_data_tables) VALUES
...
-- Create industry-specific theme variants (use INSERT IGNORE to avoid duplicates)
INSERT IGNORE INTO theme_sets (name, display_name, description, category, default_color_scheme, features, industry_features, pricing_tier, business_type_id, industry_variant, is_active) VALUES
...
SET @sql = IF(@constraint_exists = 0,
    ...
    'SELECT "business_page_sets constraint already exists"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Create view for easy industry-specific theme lookup
CREATE OR REPLACE VIEW industry_themes AS
SELECT 
    ts.id,
