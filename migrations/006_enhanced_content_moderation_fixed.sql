-- Enhanced Content Moderation Schema with Usage Intent & Appeal System (Fixed)
-- Migration: 006_enhanced_content_moderation_fixed.sql

-- Create moderation appeals table (if not exists)
...
-- Create moderation rules configuration table (if not exists)
...
-- Insert default moderation rules (ignore duplicates)
...
-- Update moderation_queue table to include appeal information (if columns don't exist)
ALTER TABLE moderation_queue 
ADD COLUMN IF NOT EXISTS appeal_id INT NULL AFTER content_moderation_id,
ADD COLUMN IF NOT EXISTS queue_type ENUM('auto_flagged', 'manual_review', 'appeal', 'admin_override') DEFAULT 'auto_flagged' AFTER priority,
ADD COLUMN IF NOT EXISTS model_id INT NULL AFTER assigned_to;

-- Add foreign keys if they don't exist
SET @sql = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
            WHERE TABLE_NAME = 'moderation_queue' AND CONSTRAINT_NAME = 'moderation_queue_ibfk_appeal');
SET @sql = IF(@sql > 0, 'SELECT "FK already exists"', 
    'ALTER TABLE moderation_queue ADD FOREIGN KEY (appeal_id) REFERENCES moderation_appeals(id) ON DELETE SET NULL');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @sql = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
            WHERE TABLE_NAME = 'moderation_queue' AND CONSTRAINT_NAME = 'moderation_queue_ibfk_model');
SET @sql = IF(@sql > 0, 'SELECT "FK already exists"', 
    'ALTER TABLE moderation_queue ADD FOREIGN KEY (model_id) REFERENCES models(id) ON DELETE CASCADE');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Create indexes for performance (if they don't exist)
CREATE INDEX IF NOT EXISTS idx_content_moderation_usage_intent ON content_moderation(usage_intent);
CREATE INDEX IF NOT EXISTS idx_content_moderation_flagged ON content_moderation(flagged);
CREATE INDEX IF NOT EXISTS idx_content_moderation_appeal_requested ON content_moderation(appeal_requested);
CREATE INDEX IF NOT EXISTS idx_content_moderation_final_location ON content_moderation(final_location);
CREATE INDEX IF NOT EXISTS idx_moderation_queue_type ON moderation_queue(queue_type);

-- Create view for admin dashboard (drop and recreate)
