-- Migration 008: Theme Sets + Modular Pages Architecture (Clean Version)
...
-- Backup existing data if needed, then drop old conflicting tables
SET FOREIGN_KEY_CHECKS = 0;

-- Drop old theme-related tables that will be replaced
DROP TABLE IF EXISTS model_theme_colors;
DROP TABLE IF EXISTS model_themes;
DROP TABLE IF EXISTS theme_colors;
DROP TABLE IF EXISTS theme_templates;
DROP TABLE IF EXISTS themes;

-- Rename existing tables to avoid conflicts
DROP TABLE IF EXISTS theme_sets;
...
-- 2. Page Types Table (Modular Pages) - Recreate with new structure
